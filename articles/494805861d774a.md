...
基本的な用語については [こちらの記事](https://zenn.dev/shougo/articles/snippet-plugins-2020) が参考になります。
...
非同期なのでまとめて呼んでもブロッキングされることはないです。
...
Placeholderでは自由に入力するだけでなく、幾つかの選択肢の中から選ぶこともできます。
...
本当は候補の一覧を表示させたいのですが、見ての通り補完プラグインとの干渉が激しいのでどうしたものか検討中です。
必要なAPIを用意して、各自の環境に合わせた設定を書いてもらうことになるかなと予想しています。

## Placeholderのネスト

Placeholderにはデフォルト値を入れることができます。
この中で、他のPlaceholderのコピーを使うことができます。
...
内部のSnippetの最後のタブストップからジャンプすると、外側のSnippetのタブストップに移動できます。
...
複数の同一タブストップがあるとき、1つ以外はそのコピーとして扱われます。
...
この加工は、そのタブストップの入力中は反映されません。
これは不完全な入力に対して、正規表現のエラーを防ぐためです。
別のタブストップにジャンプしたり、Normal modeに戻るときに反映されます。
...
`n`はタブストップのID、regexはJSの正規表現、optionは正規表現のflagsです。
...
-1にすると入力中の同期をやめます。
別のタブストップにジャンプしたりNormal modeに戻るときにだけ更新されます。
...
明日は [@wattah](https://qiita.com/wattah) さんの記事です！お楽しみに！
